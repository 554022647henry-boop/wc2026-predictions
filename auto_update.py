...
def push_to_github_pages():
    ...
    # 收集所有需要推送的本地文件 → GitHub 路径
    files_to_push = [
        (Path(config.HTML_FILE), 'web/index.html'),
    ]

    # 今日日报（如果存在就推）
    mmdd = datetime.now().strftime('%m%d')
    daily_report = Path(f'web/daily_report_{mmdd}.html')
    if daily_report.exists():
        files_to_push.append((daily_report, f'web/daily_report_{mmdd}.html'))

    for local_path, target_path in files_to_push:
